...
bool bDone = false;
const char* gpszAppName = "Vulkan Android NDK - Tessellation Shader";
int winWidth = 0;
int winHeight = 0;
    ...
    while (1) {
        ...
        if(bPendingSingleTap == true && bDoubleTapDectected == false) {
            ...
            if(now - pendigSingleTapTime > 300) { // single tap is detected after 300 milliseconds
                ...
                numLineSegments++; // increase the number of line segments to draw when single tap is detected
                if(numLineSegments >= 30){
                    numLineSegments = 30;
                }
                __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "Number of Line Segments: %d", numLineSegments);
            }
        }
...
void uninitialize(Engine* engine) {
    ...
    if(vkShaderModule_tessallation_evaluation) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "uninitialize(): vkDestroyShaderModule() Succeed for Tessellation Evalaution Shader!\n");
    ...
    if(vkShaderModule_tessallation_control) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "uninitialize(): vkDestroyShaderModule() Succeed for Tessellation Control Shader!\n");
...
VkResult createVulkanDevice () {
    ...
    if(vkPhysicalDeviceFeatures.tessellationShader == VK_TRUE) {
        ...
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createVulkanDevice(): Selected Physical Device Does Not Supports Tessellation Shader!.\n");
        return (vkResult);
    }
...
VkResult createShaders(void) {
    ...
    asset = AAssetManager_open(androidAssetManager, szFileName, AASSET_MODE_BUFFER);
    if(asset == NULL) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAssetManager_open() failed to open Tessellation Control Shader spir-v file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAssetManager_open() succeed to open Tessellation Control Shader spir-v file!.\n");
    }
    ...
    if(retVal != fileSize) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAsset_read() failed to read Tessellation Control Shader file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAsset_read() succeed to read Tessellation Control Shader file!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): vkCreateShaderModule() for Tessellation Control Shader Failed!.\n");
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): vkCreateShaderModule() for Tessellation Control Shader Successful!.\n");
    }
    ...
    __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): Tessellation Control Shader Module Created Successful!.\n");
    ...
    asset = AAssetManager_open(androidAssetManager, szFileName, AASSET_MODE_BUFFER);
    if(asset == NULL) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAssetManager_open() failed to open Tessellation Evaluation Shader spir-v file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAssetManager_open() succeed to open Tessellation Evaluation Shader spir-v file!.\n");
    }
    ...
    retVal = AAsset_read(asset, shaderData, fileSize);
    if(retVal != fileSize) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAsset_read() failed to read Tessellation Evaluation Shader file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): AAsset_read() succeed to read Tessellation Evaluation Shader file!.\n");
    }
    ...
    if(vkResult != VK_SUCCESS) {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): vkCreateShaderModule() for Tessellation Evaluation Shader Failed!.\n");
        return (vkResult);
    } else {
        __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): vkCreateShaderModule() for Tessellation Evaluation Shader Successful!.\n");
    }
    ...
    __android_log_print(ANDROID_LOG_INFO, "AMK-Ndk:", "createShaders(): Tessellation Evaluation Shader Module Created Successful!.\n");
...
VkResult createDescriptorSetLayout(void) {
    ...
    vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT 
                                                ...
                                                | VK_SHADER_STAGE_FRAGMENT_BIT; // this binding will be used in vertex shader
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        // Tessellation patch has 4 control points, so submit 4 vertices.
