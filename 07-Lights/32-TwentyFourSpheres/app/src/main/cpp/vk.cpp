...
bool bDone = false;
const char* gpszAppName = "Vulkan Android NDK - Sphere";
int winWidth = 0;
int winHeight = 0;
    ...
    while (1) {
        ...
        if(bTouchDown == true && bDragging == false && bLongPressDetected == false && bDoubleTapDectected == false)  {
            ...
            if(now - touchStartTime > 500) { // long press is detected after 5000 milliseconds
                ...
                bLight = !bLight; // toggle light on long press
            }
        }
                ...
                case AMOTION_EVENT_ACTION_DOWN: 
                {
                    ...
                    if(bPendingSingleTap == true) {
                        ...
                        if(timeSinceLastTap < 300.0f && distance < 100.0f) { // double tap is detected if second tap is within 300 milliseconds and within 100 pixels distance from first tap
                            ...
                            rotationAxis = (rotationAxis == 'x') ? 'y' : ((rotationAxis == 'y') ? 'z' : 'x'); // toggle rotation axis on double tap
                        } else {
...
VkResult createDescriptorSetLayout(void) {
    ...
    vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT; // this binding will be used in vertex and fragment shader
